...
def initial_population(data, matrix, free, filled, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    for index, classs in classes.items():
        # Shuffle free fields to pick a random starting spot
        random.shuffle(free)
        ind = 0
        while True:
            if ind >= len(free):
                # If we can't find a spot, we might need to backtrack or handle it,
                # but for now we'll just break to avoid infinite loop if data is too much
                break
...
def mutate_ideal_spot(matrix, data, ind_class, free, filled, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    # Shuffle free fields to find a random ideal spot
    random.shuffle(free)
    ind = 0
    while True:
